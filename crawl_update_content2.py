...
- 环境变量：ZHIPU_API_KEY（必填）、SQLITE_DB（默认 crawl_local.db）
...
def get_learned_json_path(conn: sqlite3.Connection, domain: str) -> Optional[str]:
    cur = conn.execute(
...
def save_learned_json_path(
    conn: sqlite3.Connection, domain: str, json_path: str, sample_url: str
) -> None:
    conn.execute(
...
def extract_main_from_json(
    ...
    if not force_relearn:
        learned = get_learned_json_path(conn, domain)
        if learned:
    ...
    if zhipu_key and domain not in domain_json_llm_asked:
        ...
        if jp:
            ...
            if isinstance(v, str) and v.strip():
                if ok_html(v):
                    save_learned_json_path(conn, domain, jp, page_url)
    ...
    learned2 = get_learned_json_path(conn, domain)
    if learned2:
...
def get_learned_xpath(conn: sqlite3.Connection, domain: str) -> Optional[str]:
    cur = conn.execute(
...
def save_learned_xpath(
    conn: sqlite3.Connection, domain: str, xpath: str, sample_url: str
) -> None:
    conn.execute(
...
def smart_extract_main_html(
    ...
    if not force_relearn:
        learned = get_learned_xpath(conn, domain)
        if learned:
    ...
    if zhipu_key and domain not in domain_llm_asked:
        ...
        if xp:
            ...
            if is_good_main_content(frag):
                save_learned_xpath(conn, domain, xp, page_url)
                return frag, "llm_xpath"
            save_learned_xpath(conn, domain, xp, page_url)
            logger.warning("[LLM] XPath 已写入库，当前页效果一般: {}…", xp[:100])

    learned2 = get_learned_xpath(conn, domain)
    if learned2:
...
def main():
    ...
    zhipu_key = (
        "" if args.no_llm else (ZHIPU_API_KEY or os.environ.get("ZHIPUAI_API_KEY", ""))
    )
    if not zhipu_key and not args.no_llm:
        logger.warning(
            "未设置 ZHIPUAI_API_KEY，大模型 XPath 不可用（可加 --no-llm 关闭本提示）"
        )
    ...
    logger.info(
        "浏览器优化 | 单页超时={}s | 图片={} | 加载模式={} | 静音=开 | JS原生弹窗=自动确定",
        pt,
    ...
    logger.info(
        "开始处理 | 本批任务={} | start={} | dry_run={} | db={} | URL去重={} | JSON优先={} | 滚底={} | 调度={}",
        total_rows,
        args.start,
        args.dry_run,
        args.db,
    ...
    def _process_row(idx: int, row: Any, cur_n: int) -> None:
        nonlocal ok, fail, ok_crawl, ok_json, ok_reuse, skip_dup, skip_ok, page
        nonlocal domain_llm_asked, domain_json_llm_asked, domains_cache_cleared
            ...
            if ex:
                ...
                if first_id == cid:
                    ...
                    if (
                        dr
                        and dr[0]
                        and valid_text_cjk_digit_alpha(dr[0]) >= MIN_TEXT_LEN
                    ):
                        if not args.dry_run:
                            conn.execute(
                                """UPDATE cms_crawl_data_content SET
        ...
        force_dom = False
        if args.force_relearn_domain and dom not in domains_cache_cleared:
            conn.execute("DELETE FROM domain_learned_xpath WHERE domain = ?", (dom,))
            conn.execute("DELETE FROM domain_json_html_path WHERE domain = ?", (dom,))
            conn.commit()
            domains_cache_cleared.add(dom)
            if dom in domain_llm_asked:
                domain_llm_asked.discard(dom)
            if dom in domain_json_llm_asked:
                ...
                if not stop_retry:
                    ...
                    if vc >= MIN_TEXT_LEN:
                        ...
                        last_err = (
                            ...
                            f"需智谱请配 ZHIPUAI_API_KEY 且勿 --no-llm"
                        )
                    else:
                        ...
                        if (
                            ...
                            stop_retry = True
                            last_err = f"页面为404/不存在类且有效字={vc}<{MIN_TEXT_LEN}，不再重试"
            except Exception as e:
